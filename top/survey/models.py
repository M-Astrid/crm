...
class Client(models.Model):
    ...
    governmental = models.BooleanField(verbose_name=u'Тип')
    subtype = models.CharField(verbose_name=u'Подтип', max_length=64, choices=ORG_SUBTYPES, blank=True)
    sanctions = models.BooleanField(verbose_name=u'Санкции', default=False)
...
class Query(models.Model):
    ...
    order_items = models.ManyToManyField(Item, verbose_name=u'Заявка', related_name='o_items', symmetrical=False)

    decline_reason = models.TextField(verbose_name=u'Причина отказа', blank=True)
    comments = models.TextField(verbose_name=u'Примечания к заказу')
